...
fn main() {
    ...
    for line in commands.split("\n") {
        ...
        let (total, (src, dst)) = line.split_once(" from ")
                                 ...
                                 .unwrap();
        //println!("{} {} {}", total, src, dst);

        let start_point = stack[src].len() - total;

        for i in start_point..stack[src].len() {
            let v = stack[src].get(i).unwrap().to_string();
            stack[dst].push_back(v);
        }
        
        for _ in 0..total {
            stack[src].pop_back().unwrap();
        }     
    }  
